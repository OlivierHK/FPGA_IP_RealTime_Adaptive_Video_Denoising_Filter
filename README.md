...
To achieve so, some novative FPGA designs had been use, like a 25-elements Batcher's odd-even mergesorte, or constant coefficient Multiplication/Division using LUTs only.
...
- Implementable code of the filter (HDL+XDC), and VHDL testbench associated can be found in the FGPA Folder.
- Matlab project code with exemple, image generation and PSNR calculation can be found in the MATLAB Folder.
...
Defect Estimator is under the form of a cross-mask centered of the 5x5 block middle pixel, where we compute the absolute maximum (T1) and Absolute minimum(T2). their value compared to fixed threshold value will tell if the block will go for CWM or not.
...
2. It will compute the difference between the second Maximum sorted pixel, and second Minimum sorted pixel. Max/Min pixel are not used to avoid to pick up an impulse noise pixel (in case there is).
3. That difference value will be compares to a set of fixed threshold (T3,T4), that will says which Gauss filters should be used, or no filter (NOP) to be used.
...
In case of High details found in the Block of pixel, no filter are applied; the center pixel is outputed directly.
...
- Will only use FGPA's LUTs, D-FlipFlop, CARRY8 and Mux as ressources. It shouls use under 6000 4-Input LUTs.
...
- coding style will be as concurrent as possible, One clock cycle will do more than one operation. FDRE will then be inferred between concurent statements to meet timing constraints for a Xilinx Virtex II (105MHz). 
- All operations are done on Fixed Length, integer numbers, represented as `std_logic_vector`:
  - Addition, Substraction, comparator will infer LUTs and CARRY8 type cells directly by the IDE tool. No DSP block will be used.
  - Multiplication/Division will be done by Left/Righ shifting and addition/substraction, using benefits of power-of-two and binary numbers. For exemple,  multiply 13 will be computed as: (2^3)+(2^2)+1 or (2^4)-(2^2)+1.
...
- Cascade of adder/substrator will be using tree-shape design to shorten critical path, increase parallelism. Temporary calculation registers have larger size to avoid overflow.
- Several stages of D-Flop to be added to time synchronized all datas betwen different module, as the design have no FSM, and is pure continuous stream.
...
### filter IP Module
This module is the top module of the filter IP. details are below.

## filter IP module

This module is the top module of the filter IP. several stages of pipeline need to be instered to control signals flow and make sure all is in sync. it ca nbe represented as :

![filter_IP_module_diagram](https://github.com/user-attachments/assets/1cc10222-7861-4421-9d42-39ed6b631249)

## Batcher's Sort

In 2008/2009, for its first release, this work was the first documented/implemented usage of a 25-elements Batcher's odd-even mergesorte network. Extensive tests been conducted to make sure it was 100% accurate. Choice been done on this algorithm as it was the best in terms of comparator needed. it requires:
- 138 comparators.
- 15 parallel operations.
- 5 pipelines stage to meetup 100MHz timing (3 comparators cascaded per clock cycle).

Since, such type of sorting has been formalized and can even be write generic parametric code. It seems is also managed to reduce the number of comparators from 138 to 130. There is room to improve further.
...
There is just some subtraction and some comparator to find the absolute maximum and the absolute minimum in the tested cross. The output is two control signals used just after. These signals are the `CWM` signal and the `Choice` signal. This last one is use to tell if the mask will be filtered by Gaussian way or CWM way. The `CWM` signal gives the information of the presence or not of a line. Because this operation is very small, it was directly coded in the `control` module.
...
Detail estimator is computed with provided algorithm ealier. `out_control` signal will recor which filter is chosen. Operationvery small, so is directly coded into the control module. 
...
Indices modules are composed by 4 times of the same module. The 5x5 block, sorted array are taken as inputs alongside with the desired ordered element index. it will output the Index of this element in the 5x5 block.

First step consist to replace every element of the 5x5 block by binary zero excepted those ones wantedd by the user in the sorted array. This number takes the binarye value of `1`.

Second step is to identify the address of highest `1`.  The Binary array is casted into a decimal number. This number is then compared to find a proportional window (with a LUT). The windows where the number corresponds its index.
...
|---|---|---|---|
```
Give the number 0010b casted to integer as 10. The LUT give for this number the value 2. The index of the element that we want is 2.

This functions is used replace the extremum values of the 5x5 block, but also find the middle element of the sorted array to insert it 14 times in the CWM filter.
...
The first step swap of the 2 second extremum Min/Max values with the help of the indices index computes from `control` module. This new Mask is then filtered.

then, Simple convolution, where we multiply each pixel by its own Gauss coefficient, add each of these values and divide by the sum of coefficients. 

Each coefficient will be decomposed into sum of power-of-two and be processed independently to shift-left values. 
...
This method introduce an error of +/-1 on a pixel value from the pixel real-value, which is acceptable in our case.
...
[FPGA]() folder contains a testbench file (`TB_test.vhd`) for post-Place and post-PAR simulation. It have a steam test vector that will test all types of filter. 

Behaviorial simulation may not work on ModelSim/ActiveHDL, as it doesn't simulate propagation delay of the logic. 
...
## projetc/Netlist generation and implementation
...
- Add a control port connected to DP-RAM that could help to:
  ...
  - Make the CWM central weigh variable, to change strength of the CWM filter.
  - optionnally make the Gauss coefficients variable to adjust the smoothing effect. This change may impact timing, and may require change in design to call DSP cells instead, as multiplication and division will not be fixed anymore.
- Make extremum swaping smarted by reordering priority ordering. actual design is just replacing the pixel with higher index in the 5x5 block.
...
- Add Filter for image's edges. It has been implemented in the MATLAB code, but not in the FPGA design (average 3x1 and 1x3).
